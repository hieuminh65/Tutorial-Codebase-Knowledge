...
# Helper to upload content to Azure Blob Storage
def upload_to_blob_storage(container_name, blob_name, content, content_type=None):
    """
    Upload content to Azure Blob Storage
    
    Args:
        container_name (str): The container name to upload to
        blob_name (str): The name of the blob (including any "folder" path)
        content (str): The content to upload
        content_type (str, optional): The content type (MIME type)
    ...
    if not connection_string:
        raise ValueError("Azure Blob Storage connection string not configured. Please set AzureWebJobsStorage in local.settings.json or Azure Portal.")
    ...
    try:
        ...
        container_client.get_container_properties()  # Will raise exception if container doesn't exist
    except Exception:
...
class CombineTutorial(Node):
    ...
    def exec(self, prep_res):
        ...
        # Determine blob storage container name based on project name
        # Using the local file path as a reference but will upload to blob storage
        project_name = os.path.basename(output_path)
        container_name = f"tutorials"
        blob_base_path = project_name
        
        print(f"Combining tutorial and uploading to Azure Blob Storage (container: {container_name}, path: {blob_base_path})")
        
        # URLs to access the files (returned to function caller)
        file_urls = []
        
        try:
            # Upload index.md
            blob_path = f"{blob_base_path}/index.md"
            ...
            for chapter_info in chapter_files:
                blob_path = f"{blob_base_path}/{chapter_info['filename']}"
            ...
            # For backward compatibility with existing code that expects a path,
            # we'll still create a minimal local version with just an info.txt file
            # pointing to the blob storage files
            try:
                # Create local directory for minimal info
                os.makedirs(output_path, exist_ok=True)
                
                # Write info.txt with URLs
            ...
            # Return both the blob storage base URL and the local path
            ...
            # Fallback to local filesystem if blob storage fails
            os.makedirs(output_path, exist_ok=True)
